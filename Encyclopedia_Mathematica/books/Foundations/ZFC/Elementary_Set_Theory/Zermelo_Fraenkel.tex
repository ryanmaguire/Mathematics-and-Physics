...
\section{The Axioms of Zermelo and Fraenkel}
    ...
    \subsection{Subsets and Equality}
        ...
        \begin{example}
            ...
            mean for $n=0$. From the way we will define this, $\mathbb{Z}_{0}$
            is empty. We will say $k\in\mathbb{Z}_{n}$ if $k\in\mathbb{N}$ and
            $k<n$, a description that will be justified by the axiom schema of
        ...
        \begin{example}
            ...
            \textit{axiom of extensionality} to ease the demonstration of
            equality.
        \end{example}
        ...
        \begin{faxiom}{Axiom of Extensionality}{Axiom_of_Extensionality}
            ...
            \begin{equation*}
                \forall_{A}\forall_{A}\Big(\forall_{x}(x\in{A}\Leftrightarrow
        ...
        \begin{example}
            ...
            (see Ax.~\ref{ax:Axiom_Schema_of_Specification}).
            Then $x\in{A}$ if and only if $x>0$. That is, $A$ is the set of all
            positive real numbers. But if $x>0$, then $x\ne{0}$ and $x$ is
            non-negative. That is, $x\not\leq{0}$. But then $x$ satisfies the
            criterion for membership of $B$, and therefore $x\in{B}$. Similarly,
            $y\in{B}$ if and only if $y\not\leq{0}$ and this is just another way
            of stating that $y>0$, and hence $y\in{A}$. Thus, by the axiom of
            extensionality, $A=B$.
        \end{example}
        We'll restate the definition of equality using the language of
        subsets\index{Set!Subset}, lessening the effort required in proving
        various things are equal. The notions are equivalent. Subsets are sets
        that are defined in terms of another given set by simply removing some
        (or none, or all) of the elements.
        ...
        \begin{example}
            ...
            \end{equation}
            Letting \gls{mathbbQ} denote the rational numbers $p/q$, where
            ...
            complex numbers (\gls{mathbbC}) as well, creating a chain of subsets:
            \begin{equation}
        ...
        containment (\gls{containmentsymb}) for for subset (\gls{subseteq}).
        The symbol $\in$ is used to denote that some object $x$ is an
        \textit{element} \index{Set!Element of} of some set. That is, $x\in{A}$
        indicates that $x$ is an element of $A$. This does not necessarily imply
        ...
        \begin{fdefinition}{Proper Subsets}{Proper_Subsets}
            ...
            subset of $B$.\index{Proper Subset}
            \begin{equation*}
                \forall_{A}\forall_{B}(A\subsetneq{B})
                \Longleftrightarrow(A\subseteq{B}\land{A}\ne{B})
            \end{equation*}
        \end{fdefinition}
        ...
        loosely defined then natural numbers \gls{mathbbN} and the integers
        \gls{mathbbZ} (From the German \textit{Zahl}) by
    ...
    \subsection{Ordered Pairs and Unions}
        ...
        \begin{fdefinition}{Ordered Pairs}{Ordered_Pairs}
            ...
            \end{equation*}
        \end{fdefinition}
        Kuratowski\index{Kuratowski, Kazimierz} first put forward this
        definition in 1921 and this does precisely what we want it to do and
        orders elements. That is, if $x$ and $y$ are distinct, then
        ...
        \begin{fdefinition}{Union over a Set}{Union_over_a_Set}
            ...
            \begin{equation*}
                \forall_{\mathcal{O}}\forall_{x}\Big(
        ...
        \begin{proof}
            ...
            $A\cup{B}$ such that $x\in{A}\cup{B}$ if and only if there exists a
            set $\mathcal{U}\in\mathcal{O}$ such that $x\in\mathcal{U}$. But
            then $x\in{A}\cup{B}$ if and only if either $x\in{A}$ or $x\in{B}$.
        \end{proof}
        ...
        sets we have slightly abused our set-builder notation. The axiom schema
        ...
        both $A$ and $B$ simultaneously. That is, $x\in{A}\cap{B}$ if and only
        if $x\in{A}$ \textbf{and} $x\in{B}$. We can similarly define
        intersections over arbitrary collections, denoted \gls{intersectO}.
        \begin{theorem}
        ...
        \begin{example}
            ...
            (see Thm.~\ref{thm:Intersection_of_Subset} and
        ...
        \begin{theorem}
            ...
            $x\in\bigcap\mathcal{O}$ if and only if $x\in\bigcup\mathcal{O}$ and
        ...
        to set theorists. Nevertheless, it is vital to support the claim that
    ...
    \subsection{Cartesian Products and Functions}
        ...
        \index{Euclid of Alexandria} and Ren\'{e} Descartes%

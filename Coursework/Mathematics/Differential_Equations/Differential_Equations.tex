    ...
    \section{Systems of Differential Equations}
        \subsection{Linear First Order Systems}
            ...
            \begin{bproof}
                ...
                $r^{2}+ar+b$, and therefore
                $\ddot{x}_{1}(t)+a\dot{x}_{1}(t)+bx_{1}(t)=0$.
                Similarly, $x_{2}(t)$ satisfies the differential
                equation since $r_{2}$ is also a root of the
                polynomial $r^{2}+ar+b$. And since the Wronskian
                of $y_{1}$ and $y_{2}$ is non-zero, any solution
                is of the form:
                \begin{equation*}
                ...
                \begin{align*}
                    ...
                    &=\dot{x}_{0}
                \end{align*}
                The coefficient matrix of this system is invertible
                as $a^{2}-4b>0$. Solving this system, we have:
                \begin{align*}
                    C_{1}&=
                    \frac{1}{2\sqrt{a^{2}-4b}}\Big(
                    \big(a+\sqrt{a^{2}-4b}\big)x_{0}+
            ...
            \begin{bproof}
                ...
                \begin{equation*}
                    ...
                    \end{cases}
                \end{equation*}
                Here $n\mod{k}$ means the \textit{remainder}
                of $n/k$. We can therefore split the sum
                into two parts:
                \begin{align*}
                    \exp(i\theta)
                    &=\sum_{n=0}^{\infty}\frac{(i\theta)^{n}}{n!}\\
            ...
            \begin{bproof}
                ...
                Let $x_{1}(t)=\exp(-at/2)\cos(\sqrt{4b-a^{2}}t/2)$
                and $x_{2}(t)=\exp(-at/2)\sin(\sqrt{4b-a^{2}}t/2)$.
                ...
                $x(t)$ and $\dot{y}(x)$ at $t=0$ gives us the
            ...
            found a first one. Before we get there, let's try wildly
            ...
            non-zero. We can now write the general solution for
            the degenerate case.
            ...
            \begin{equation}
                ...
                v(t)=\dot{u}(t)
            \end{equation}
            Recall that $x_{1}(t)$ is not identically zero everywhere.
            Thus we can convert
            Eqn.~\ref{diffeq:Reduc_of_Order_Reduced_Equation_in_u} into
            the following first-order linear equations in $v(t)$:
            \begin{subequations}
                \begin{align}
                    x_{1}(t)\dot{v}(t)+
            ...
            We can simplify $\mu(t)$ further by evaluating part of
            the integral.
            \begin{subequations}
                \begin{align}
            ...
            \textit{Forcing Function}. The classic example, which we will
            consider later, is a damped harmonic oscillator in which
            $p(t)$ and $q(t)$ are constants, and $F(t)$ is some function that
            continuously perturbs the system. If $x_{1}(t)$ is a solution to
            Eqn.~\ref{diffeq:general_second_order_linear_DE_non_hom}, then
            a second independent solution $x_{2}(t)$ can be found as follows:
            \begin{equation}
                x_{2}(t)=
                x_{1}(t)\int_{t_{0}}^{t}\bigg[\frac{1}{x_{1}(\tau)^{2}}
                \exp\Big(-\int_{t_{0}}^{\tau}p(\xi)\diff{\xi}\Big)
            ...
            \begin{equation}
                z(t)=x(t)+iy(t)
            \end{equation}
            Where $x(t)$ and $y(t)$ are real-valued functions. Plugging this into
            Eqn.~\ref{diffeq:Sec_Ord_Lin_Const_Complex_Coeff} we obtain
            the following second-order linear differential equations with
            real constant coefficients:
            \newline
            \begin{subequations}
                \begin{tabularx}{\textwidth}{@{}XX@{}}
                    \begin{equation}
                        \ddot{x}(t)+a_{1}\dot{x}(t)+a_{2}x(t)=0
                    \end{equation}
                    &
                    \begin{equation}
                        \ddot{y}(t)+b_{1}\dot{y}(t)+b_{2}y(t)=0
                    \end{equation}
                \end{tabularx}
            \end{subequations}
            We can solve for both $x(t)$ and $y(t)$ by invoking
            Eqn.~\ref{diffeq:gen_sol_to_sec_ord_lin_de_const_coeff}.
            Thus, complex coefficients don't make the problem any
            more difficult. As a final example, we consider the phenomena
            of coupled oscillations.
            \begin{fexample}{Coupled Oscillators}{diffeq_coupled_oscillators}
                Consider two particles $a(t)$ and $b(t)$ that satisfy the
                ...
                \begin{equation}
                    ...
                    \end{bmatrix}
                \end{equation}
                We have seen before that given such a system we can rewrite
                this as two ordinary (uncoupled) second-order linear
                differential equations by considering the trace and the
                determinant of the coefficient matrix. The trace is
                $-2i\beta$ and the determinant is $\kappa^{2}-\beta^{2}$.
                So, we have:
                \begin{subequations}
                    \begin{align}
                    ...
                    \end{align}
                \end{subequations}
                Consider the case of $\beta$ and $\kappa$ being real numbers,
                and given the initial conditions $a(0)=1$ and $b(0)=0$. The
                general solution is:
                \begin{subequations}
                    \begin{align}
                        a(z)&=\big(A_{1}\cos(\kappa{t})+A_{2}\sin(\kappa{t})\big)
                        \exp\big(-i\beta{t}\big)\\
                        b(z)&=\big(B_{1}\cos(\kappa{t})+B_{2}\sin(\kappa{t})\big)
                        \exp\big(-i\beta{t}\big)
                    \end{align}
                \end{subequations}
                We have $a(0)=1$, and thus $A_{1}=1$. But also
                $b(0)=0$, and therefore $B_{1}=0$. To solve for $A_{2}$
                and $B_{2}$ we need to use
                Eqn.~\ref{diffeq:coupled_osc_eq_1} and
                Eqn.~\ref{diffeq:coupled_osc_eq_2}. That is, we have
                $\dot{a}(0)=-i\beta{a}(0)-i\kappa{b}(0)=-i\beta$.
                Similarly, $\dot{b}(0)=-i\beta{b}(0)-i\kappa{a}(0)=-i\kappa$.
                Therefore $A_{2}=0$ and $B_{2}=-i$. The solution is then:
                \begin{subequations}
                    \begin{align}
                        a(t)&=\cos(\kappa{t})\exp\big(-i\beta{t}\big)\\
                        b(t)&=-i\sin(\kappa{t})\exp\big(-i\beta{t}\big)
                    \end{align}
                \end{subequations}
                Using Euler's Formula
                (Thm.~\ref{th:diffeq:Euler_Formula}), we can write this as:
                \begin{subequations}
                    \begin{align}
                    ...
                    \end{align}
                \end{subequations}
                The real part of both $a(t)$ and $b(t)$ will thus be two waves
                of wavelengths $\beta$ and $\kappa$ superimposed on each other.
                The lower frequency term is often called the
                \textit{envelope} of the wave. For example if $\beta=1$
                and $\kappa=10$, we have the following plots:
                \begin{figure}
                    ...
                    \end{subfigure}
                    \caption{Real Parts of the Solutions to the
                             Initial Value Problem. The dashed lines
                             represent the \textit{envelopes}.}
                    \label{fig:my_label}
                \end{figure}
            \end{fexample}
\end{document}
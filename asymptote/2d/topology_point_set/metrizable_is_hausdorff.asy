...
real dx = xpt.x - ypt.x;
real dy = xpt.y - ypt.y;
real radius = 0.5*sqrt(dx*dx + dy*dy);
...
draw(g);
filldraw(circle(xpt, radius), Upen, dashpen);
filldraw(circle(ypt, radius), Vpen, dashpen);

/*  Draw dots for the points x and y.                                         */
filldraw(circle(xpt, rDot), black, black);
filldraw(circle(ypt, rDot), black, black);

draw("$\varepsilon$", xpt -- (xpt + scale(radius)*expi(0.7*pi)));
draw("$\varepsilon$", ypt -- (ypt + scale(radius)*expi(0.3*pi)));

/*  Label X, U, V, and the points x and y.                                    */
label("$x$", xpt, SE);
label("$y$", ypt, SE);
label("$X$", (1.7, 0.8));
label("$\mathcal{U}$", xpt + (-0.3*radius, -0.4*radius));
label("$\mathcal{V}$", ypt + (-0.3*radius, -0.4*radius));
label("$\varepsilon=\frac{1}{2}d(x,\,y)$", (2.9, 0.6));

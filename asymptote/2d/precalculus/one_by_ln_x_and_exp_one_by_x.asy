...
/*  The function to be plotted, 1 / ln(x).                                    */
real f(real x)
{
    return 1.0 / log(x);
}
...
/*  Function used to avoid plotting the points where f_inv is undefined.      */
bool3 branch_inv(real x)
{
    if (x == 0.0)
        return false;
    else
...
draw(graph(f_inv, -xlen, xlen, n=n_samples, branch_inv), deepred);
...
/*  Label the function.                                                       */
label("$f(x)=\frac{1}{\ln(x)}$", (6.0, 2.0), deepblue);
label("$f^{-1}(x)=\exp\big(\frac{1}{x}\big)$", (-5.0, 2.0), deepred);
draw((-xlen, -xlen) -- (xlen, xlen), dashed);

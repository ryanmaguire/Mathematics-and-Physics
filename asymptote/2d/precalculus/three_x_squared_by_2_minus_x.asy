...
/*  The function to be plotted, 3x^2 / (2 - x).                               */
real f(real x)
{
    real denom = 2.0 - x;
    return 3.0*x*x / denom;
}
...
/*  Graph the function.                                                       */
draw(graph(f, -xlen, xlen, n=n_samples, branch, operator ..), deepblue);
...
/*  Label the function.                                                       */
label("$f(x)=\frac{3x}{\frac{2}{x}-1}$", (-2.0, 20.0), deepblue);

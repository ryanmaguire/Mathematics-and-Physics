...
/*  This edge overlaps with the previous, so overwrite with white to give the *
 *  illusion of one edge passing over the other.                              */
draw(B[2] .. (B[1] - (0.0, 0.25)) .. A[0], white + linewidth(2.0pt));
draw(B[2] .. (B[1] - (0.0, 0.25)) .. A[0]);

/*  The rest of the edges are straight lines.                                 */
draw(A[0] -- B[0]);
draw(A[0] -- B[1]);
draw(A[1] -- B[1]);
draw(A[1] -- B[2]);

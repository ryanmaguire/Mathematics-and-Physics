...
/*  Incrementing factor for the homotopy.                                     */
real ds = 1.0 / n_frames;
...
for (n = 0; n <= n_frames; ++n)
{
    ...
    draw(surf, surfacepen = blob, render(merge = true));
    draw(tube(boundary(), scale(0.02)*unitcircle), surfacepen=pipe);

...
/*  Setup the camera.                                                         */
currentprojection = perspective(2.0, 2.0, 2.0);
...
/*  Pen for the radial line from the origin to the sphere.                    */
pen drawp = blue + linewidth(0.8pt) + fontsize(24pt);

/*  Start and end radii for the sphere over the course of the GIF.            */
...
/*  The material the sphere is made out of.                                   */
...
/*  Point for the origin and the position vector on the sphere.               */
triple O = (0.0, 0.0, 0.0);
real RCPR_SQRT_2 = 1.0 / sqrt(2.0);
triple r_vector = (-RCPR_SQRT_2*r, RCPR_SQRT_2*r, RCPR_SQRT_2*r);
...
/*  Loop over the radii and draw the sphere growing.                          */
for (n = 0; n < n_frames; ++n)
{
    /*  Save the image without any sphere drawn yet.                          */
    save();

    /*  Draw the sphere and the position vector.                              */
    draw(scale(r, r, r)*unitsphere, surfacepen=blob, render(merge=true));
    draw(L, O -- scale(r, r, r)*r_vector, drawp, Arrow3);
...
/*  End of for-loop over the radius of the sphere.                            */

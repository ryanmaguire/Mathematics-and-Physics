...
// Draw the sphere.
g = A{dir(30)}..F..D..G..{dir(150)}B--
    shift(midpoint(A, B))*(scale(0.3, 1.0)*arc(O, r, 270, 450))--cycle;
...
axialshade(g, gray(0.5)+opacity(0.4), O, white+opacity(0.5), C);
draw(g);

// Add elliptical lines making it look 3D.
g = scale(0.3, 1.0)*arc(O, r, 270, 450);
draw(shift(midpoint(A, B))*g);

g = scale(0.3, 1.0)*arc(O, r, 90, 270);
draw(shift(midpoint(A, B))*g, linetype("4 4"));

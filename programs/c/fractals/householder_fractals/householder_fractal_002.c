 ...
 *  Date:   2021/02/13                                                        *
 ******************************************************************************
 *  This code creates a Householder fractal for polynomials up to degree 14.  *
 ...
 *  This algorithm finds the roots, and then Householder's method is used to  *
 *  create a Householder fractal. This code has been greatly generalized and  *
 *  made C89/C90 compliant in my other project, libtmpl. The code presented   *
 *  here requires C99 features, in particular complex.h.                      *
 *                                                                            *
 *  NOTE:                                                                     *
...
/*  Maximum number of iterations for Householder's method. This must be       *
...
/*  Computes the third derivative of the polynomial.                          */
static complex double poly_triple_prime_eval(complex double z)
{
    /*  Variable for indexing.                                                */
    unsigned int n;

    /*  Initialize the output to deg * (deg-1) * (deg-2) * a_{0}.             */
    complex double out = DEG*(DEG-1U)*(DEG-2U)*coeffs[0];

    /*  Compute the third derivative of the polynomial via Horner's method.   */
    for (n = 1U; n < DEG - 2U; ++n)
        out = z*out + (DEG - n)*(DEG - n - 1U)*(DEG - n - 2U)*coeffs[n];

    return out;
}
/*  End of poly_triple_prime_eval.                                            */
...
/*  Function for drawing the Householder fractal of the input polynomial.     */
int main(void)
{
    /*  Declare all necessary variables.                                      */
    FILE *fp;
    complex double f_z, df_z, d2f_z, d3f_z, z, roots[DEG];
    ...
    for (y = 0U; y < size; ++y)
    {
        ...
        for (x = 0U; x < size; ++x)
        {
            ...
            /*  Allow max_iters number of iterations of Householder's method. */
            for (iters = 0U; iters < max_iters; ++iters)
            {
                /*  Perfrom Householder's Method on the polynomial f.         */
                f_z = poly_eval(z);
                df_z = poly_prime_eval(z);
                d2f_z = poly_double_prime_eval(z);
                d3f_z = poly_triple_prime_eval(z);
                z = z - 3.0*f_z * (2.0*df_z*df_z - f_z*d2f_z) /
                    (6.0*df_z*df_z*df_z - 6.0*f_z*df_z*d2f_z + f_z*f_z*d3f_z);

 ...
 *  hence the choice of this function. A full color wheel                     *
 *  blue -> blue-green -> green -> yellow -> red -> purple -> blue is used to *
 *  remove the appearance of a discontinuity in the function. That is, the    *
...
{
    struct color out;
    double x = (double)c.red;
    double y = (double)c.green;
    double z = (double)c.blue;
    double r = 255.0 / sqrt(x*x + y*y + z*z);
...
static struct color get_color(struct complex_number z)
{
    ...
    return scale_color(normalize_color(out), t);
}
/*  End of get_color.                                                         */
...
int main(void)
{
    ...
    FILE *fp = fopen("z3_minus_1_over_3z2_color_wheel_normalized.ppm", "w");

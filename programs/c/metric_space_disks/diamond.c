 ...
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Draw a diamond with a black-to-white gradient.                        *
 ******************************************************************************/
...
/*  Function for drawing a diamond with a black-to-white gradient.            */
int main(void)
{
    ...
    struct msd_ppm PPM = msd_ppm_create("diamond.ppm");
...
#pragma omp parallel for
    ...
    for (n = 0U; n < size; ++n)
    {
        ...
        /*  Compute the distance using the Manhattan metric.                  */
        const double dist = fabs(x) + fabs(y);

        /*  Points outside of the diamond are white.                          */
        if (dist > 1.0)
            c[n] = msd_white;

        /*  Points inside the diamond are given a white-to-black gradient.    */
        else
            c[n] = msd_color_scale(&msd_white, 1.0 - dist);
    }

...
static const double root_toler = 1.0E-10;
...
static complex double square_to_disk_homeo(complex double z)
{
    return z / (1.0 - cabs(z));
}
/*  End of square_to_disk_homeo.                                              */
...
int main(void)
{
    ...
    fp = fopen("laguerre_fractal_plane_to_square_001.ppm", "w");
    ...
    for (y = 0U; y < size; ++y)
    {
        ...
        for (x = 0U; x < size; ++x)
        {
             ...
             *  homeomorphism to stretch the disk into the entire plane.      */
            z = square_to_disk_homeo(z_x + dbl_I*z_y);
            ...
            for (iters = 0U; iters < max_iters; ++iters)
            {
                ...
                f_z = poly_eval(z);
                G = poly_prime_eval(z) / f_z;
                H = G*G - poly_double_prime_eval(z)/f_z;
            ...
            if ((min > dist_toler) || (z_x*z_x + z_y*z_y >= 1.0))

...
\title{MIT PRIMES: Analyzing the Scattered Signal from the Cassini Mission}
\author{Ryan Maguire}
\date{\today}
...
\begin{document}
    ...
    \section{The Problem}
        ...
        During PRIMES 2025 we developed new techniques to greatly improve the
        reconstruction methods. The data available on the NASA PDS is limited
        to a 1 kilometer resolution, but our new methods allow 0.055 kilometer
        resolutions to be accurately obtained, and even finer profiles are
        possible in certain data sets. We now have the task ahead of us of
        \textit{using} these new techniques to actually analyze the rings and
        discover new science. We are particulanewrly concerned with analyszing
        the scattered signal from the rings.

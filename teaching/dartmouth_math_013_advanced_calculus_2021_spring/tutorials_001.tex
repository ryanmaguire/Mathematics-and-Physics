...
\begin{document}
    ...
    \begin{example}[\textbf{Change of Basis, Reflections Using Angles}]
        ...
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}
        Uh oh, we got something very different before. What went wrong?
        ...
        $g\big((1,\,0)\big)=\big(-\sin(\theta),\,\cos(\theta)\big)$ and
        $g\big((0,\,1)\big)=\big((\cos(\theta),\,\sin(\theta)\big)$, so the
        ...
        \begin{align}
            ...
            &=\sin(2\theta)\cos(\theta)-\cos(2\theta)\sin(\theta)
        \end{align}
        We have also used the fact that $\cos$ is even and $\sin$ is odd in
        our simplifying.
    \end{example}
    \begin{example}[\textbf{Image of Linear Subspaces}]
        ...
        $a,b\in\mathbb{R}$. You could similarly define a linear subspace to be a
        ...
        $\tilde{\mathbf{x}}+\tilde{\mathbf{y}}\in{f}[\mathcal{C}]$.
    ...
    \begin{example}[\textbf{Affine Transformations}]
        ...
        \begin{align}
            ...
            &=L(\mathbf{p})+L(t\mathbf{v})+\mathbf{b}
                &\tag{Linearity}\\
            &=\big(L(\mathbf{p})+\mathbf{b}\big)+L(t\mathbf{v})
                \tag{Rearrange the Sum}\\
            &=\big(L(\mathbf{p})+\mathbf{b}\big)+tL(\mathbf{v})
                \tag{Linearity}
        \end{align}
        Since $\mathbf{p}\in\mathbb{R}^{n}$ is a fixed constant vector,
        $L(\mathbf{p})\in\mathbb{R}^{m}$ is also a fixed constant vector.
        Hence the sum $L(\mathbf{p})+\mathbf{b}$ is a fixed element of
        $\mathbb{R}^{m}$. Label this $\mathbf{c}$. That is,
        $\mathbf{c}=L(\mathbf{p})+\mathbf{b}$. Similarly, label
        $\mathbf{d}=L(\mathbf{v})$. Thus, for every $\mathbf{x}\in\mathcal{C}$,
        we have that $f(\mathbf{x})=\mathbf{c}+t\mathbf{d}$ for some
        $t\in\mathbb{R}$. The set:
        \begin{equation}
            \mathcal{D}=\{\,\mathbf{c}+t\mathbf{d}\;|\;t\in\mathbb{R}\,\}
        \end{equation}
        is an affine subset of $\mathbb{R}^{m}$, and we have just shown
        that $f[\mathcal{C}]=\mathcal{D}$. That is, the affine image of an
        affine subspace is also an affine subspace.
    \end{example}
\end{document}

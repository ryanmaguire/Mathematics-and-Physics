...
\begin{document}
    ...
    \section{Applications of Legendre Polynomials}
        ...
        \begin{equation}
            ...
            F_{n}(\alpha,\,\beta)
            =\frac{P_{n}-\alpha{P}_{n+1}(\alpha)}{n+2}
                -\beta\sum_{k=0}^{n}P_{n+1-k}(\alpha)P_{k+1}(\alpha)
        \end{equation}
        where:
        \begin{equation}
            \beta=
    ...
    \section{Appendix A: Fresnel Approximation via \texttt{sympy}}
...
print(psi_simplify)
    ...
    \section{Appendix B: Elliptical Rings}
\end{document}

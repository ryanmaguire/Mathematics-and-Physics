...
\begin{document}
    ...
    \section{Extended Gauss Code}
        ...
        exactly twice, one with an $t=O$ and once with a $t=U$, and the sign
    ...
    \section{Virtual Knots}
        ...
        higher genus surface. Let's try the torus $T^{2}$.
        \begin{figure}
            \centering
        ...
        \end{figure}
        The label \textit{chain-link-fence-knot} comes from trying to draw it
        on the flat torus
    ...
    \section{Resolving a Crossing}
        ...
        we write $k$ out in binary. The value of this $m^{th}$ entry of the
        binary representation of $k$ corresponds to how the $m^{th}$ crossing
        is smoothed. Take the right handed trefoil as an example with its

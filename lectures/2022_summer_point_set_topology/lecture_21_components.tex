...
\begin{document}
    ...
    \section{Connected and Path Connected Components}
        ...
        \begin{proof}
            ...
            $A\in\mathcal{O}$ and hence $(A,\,\tau_{A})$ is connected.
            So $(C,\,\tau_{C})$ is connected.
        \end{proof}
        ...
        \begin{proof}
            ...
            $\mathcal{C},\mathcal{D}\subseteq\textrm{Cl}_{\tau}(C)$ such that
            $\mathcal{C}\cup\mathcal{D}=\textrm{Cl}_{\tau}(C)$. But
            $\textrm{Cl}_{\tau}(C)$ is closed so $\mathcal{C}$ and $\mathcal{D}$
            are closed with respect to $\tau$. But then
            $\mathcal{U}=X\setminus\mathcal{C}$ and
            $\mathcal{V}=X\setminus\mathcal{D}$ are disjoint open sets in
            $\tau$. But then $\mathcal{U}\cap{C}$ and
            $\mathcal{V}\cap{C}$ are disjoint non-empty open subsets with
            respect to $\tau_{C}$ that separate $C$, a contradiction since
            $(C,\,\tau_{C})$ is connected. Hence,
            $(\textrm{Cl}_{\tau}(C),\,\tau_{\textrm{Cl}_{\tau}(C)})$ is
            connected.
        \end{proof}
        \begin{theorem}
        ...
        \begin{definition}[\textbf{Path Connected Component}]
            ...
            \begin{equation}
                C=\big\{\,y\in{X}\;|\;\exists_{f:[0,\,1]\rightarrow{X}}
                    \big(f\textrm{ continuous and }f(0)=x,\,f(1)=y\,\big\}
            \end{equation}
    ...
    \section{Arc Connected}
        ...
        \begin{example}
            ...
            twice. It's impossible to do this injectively. You can't just
            jump from $0_{0}$ to $0_{1}$, that is not continuous. There is
            no \textit{next} positive real number you can go to either.
            Any path from $0_{0}$ to $0_{1}$ is not injective. This space

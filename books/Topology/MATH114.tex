...
\begin{document}
    ...
    \pagenumbering{arabic}
        ...
        \section{Cell Complexes}
            ...
            \end{minipage}
            \par\hfill\par
            Nevertheless, it is useful for visualizations. The resulting object
            is called the \textit{cross cap} (Fig.~\ref{fig:Cross_Cap}).
            \par
            \begin{minipage}[b]{0.56\textwidth}
                The cross cap intersects itself, as it must since it is
                impossible to embed $\mathbb{RP}^{2}$ into $\nspace[2]$. It is
                perhaps difficult to see this, so a wireframe representation is
                given in Fig.~\ref{fig:Inside_Cross_Cap}. The pinch point
                present in this figure means that this is not an
                \textit{immersion}. David Hilbert posed a question to Werner Boy
                asking him to prove no such immersion exists. Instead, Boy came
                up with a way to do it, the resulting surface now known as the
                \textit{Boy surface}. Topologically, these are all just real
                projective planes. Higher dimensional analogs of this result in
                the real projective space $\mathbb{RP}^{n}$.
            \end{minipage}
            \hfill
            \begin{minipage}[b]{0.42\textwidth}
                \centering

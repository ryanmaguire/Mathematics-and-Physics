\section{The Structure of Sets}
    ...
    can be proven in an algebraic setting by using this structure and as such it
    is our current goal to prove the basics about unions and intersections so we
    we may transition to algebra.
    ...
    \subsection{Operations on Sets}
        ...
        \end{proof}
        When taking the union of two sets, we obtain a \textit{larger} set.
        In arithmetic, given two non-negative integers $a$ and $b$, it is true
        that $a\leq{a}+b$. Equality is obtained if and only if $b$ is equal to
        zero. The empty set thus acts as the \textit{identity} for unions.
        \begin{theorem}
        ...
        \begin{proof}
            ...
            contradiction.
        \end{proof}
        For real numbers $a$, $b$, and $c$, if $b\leq{c}$, then $a+b\leq{a}+c$.
        A similar result will hold for unions.
        \begin{theorem}
        ...
        prove that the associative law is true for both operations. Once we've
        proven some results about intersections, we will return and prove the
        associative laws.
        \begin{ltheorem}{Commutative Law of Intersections}{Commut_Law_Intersec}
            If $A$ and $B$ are sets, then $A\cap{B}=B\cap{A}$.
        \end{ltheorem}
        \begin{proof}
            For if $x\in{A}\cap{B}$, then $x\in{A}$ and $x\in{B}$. But then
            $x\in{B}$ and $x\in{A}$, and therefore $x\in{B}\cap{A}$

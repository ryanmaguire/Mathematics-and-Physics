\section{What is Logic?}
    ...
    \begin{example}
        ...
        theorem\index{Theorem!Intermediate Value}. Fear not those who don't, we
        ...
        and 1 to a positive number, then there is some number in the middle
        which evaluates to zero. The method of proof is quite simple: We first
        look at what happens to the point $\frac{1}{2}$. If $f$ is zero here,
        we are done, otherwise if $f$ evaluates to a positive number then we may
        suspect there's a point in between 0 and $\frac{1}{2}$ that evaluates to 
        zero. If $f$ is negative at this point, then there's probably a zero in
        between $\frac{1}{2}$ and 1. In either case we divide the range of
        possibilities in half and see what happens at $\frac{1}{4}$ in the first
        case and $\frac{3}{4}$ in the latter. We continue \textit{inductively}
        (whatever this means) and obtain a \textit{sequence} of real numbers
        which we then show \textit{converge} to some real number between 0 and
        1. We then invoke continuity to show that $f$ evaluates to zero at this
        point, and we are done (see Fig.~\ref{fig:Sketch_of_IVP}).
    \end{example}
    \begin{figure}[H]
        \centering
    ...
    \begin{example}
        ...
        mathematical calculations at the age of three. Alas, we're a few stories
        away from forming the religion of Gauss. Nevertheless, let's see what
        the argument is. We rearrange this sum as follows:
        \begin{table}[H]
            \centering
    ...
    \begin{example}
        ...
        studied sums and considere the bizarre series $1+2+3+4+\dots$ arriving
        ...
        \end{table}
        And hence $T=\frac{1}{4}$. With this, we now look to Euler's sum of
        ...
        \begin{equation}
            S-T=4S
        \end{equation}
        But $T=\frac{1}{4}$, so we have $3S=\minus{1}{4}$ and therefore
        $S=\minus\frac{1}{12}$.
    \end{example}
    We now contemplate the previous examples and ask which were valid. It is
    ...
    garbage. For the rest of the chapter we wish to hone in on what we are to
    consider as valid mathematics.
    \subsection{Truth}

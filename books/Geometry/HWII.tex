...
\begin{document}
    ...
    \section{Homework II}
        ...
        \begin{solution}
            ...
            Conversely, suppose
            $F^{*}[\Ckspace{\infty}{N}]\subseteq\Ckspace{\infty}{M}$. Let
            $(\mathcal{V},\psi)\in\mathcal{A}_{N}$ be a chart the contains
            $F(p)$ and let $(\mathcal{U},\varphi)\in\mathcal{A}_{M}$ be a chart
            that contains $p$. Suppose $\psi\circ{F}\circ\varphi^{\minus{1}}$
            is not smooth. Then there is some component $k$ such that
            the projection map $\pi_{k}:\nspace[m]\rightarrow\nspace[]$
            composed with this is not smooth. That is, defining
            $f:\nspace\rightarrow\nspace[]$ by:
            \begin{equation}
                f=\pi_{k}\circ\psi\circ{F}\circ\varphi^{\minus{1}}
            \end{equation}
            is not smooth. But $\pi_{k}\circ\psi:N\rightarrow\nspace[]$ is the
            composition of smooth functions, and is hence smooth. But by
            hypothesis, for all $g\in\Ckspace{\infty}{N}$,
            $F^{*}(g)\in\Ckspace{\infty}{M}$. But then
            $\pi_{k}\circ\psi\circ{F}$ is smooth. But $\varphi^{\minus{1}}$ is
            smooth, and hence $\pi_{k}\circ\psi\circ{F}\circ\varphi^{\minus{1}}$
            is smooth, a contradiction.
        \end{solution}
\end{document}
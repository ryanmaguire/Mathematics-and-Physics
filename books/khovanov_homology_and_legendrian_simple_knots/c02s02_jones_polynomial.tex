\section{Jones Polynomial}
    ...
    \subsection{An Explicit Algorithm Using Gauss Code}
        ...
        \begin{theorem}
            There is an algorithm for extended Gauss code with time complexity
            $O(\textrm{poly}(N)\cdot{2}^{N})$ and spatial complexity
            $O(N)$ such that the worst, average, and best cases in time are
            identical.
        \end{theorem}
        If we assume certain arithmetic operations are constant, then
        $\textrm{poly}(N)$ can be replaced by $N$.
        \par\hfill\par
